...
def topo_log_count(act_id, topo_type):
    if topo_type == "L3_Topo_Type":
        topo_end_msg = db.BenchmarkSummaryLog.find_one({'dataSourceId': act_id, 'msg': {'$regex': 'End: build IPv4 L3 Topology'}})
        if topo_end_msg:
        ...
        topo_end_msg = db.BenchmarkSummaryLog.find_one({'dataSourceId': act_id, 'msg': {'$regex': 'End: build L2 Topology'}})
        if topo_end_msg:
            topo_end_log = topo_end_msg.get('msg')
        ...
        topo_end_msg = db.BenchmarkSummaryLog.find_one({'dataSourceId': act_id, 'msg': {'$regex': 'End: build IPv6 L3 Topology'}})
        if topo_end_msg:
            topo_end_log = topo_end_msg.get('msg')
            topo_count = get_log_info(topo_end_log, 6)
            return topo_count
        ...
        topo_end_msg = db.BenchmarkSummaryLog.find_one({'dataSourceId': act_id, 'msg': {'$regex': 'End: build L3 VPN Tunnel'}})
        if topo_end_msg:
            topo_end_log = topo_end_msg.get('msg')
        ...
        topo_end_msg = db.BenchmarkSummaryLog.find_one({'dataSourceId': act_id, 'msg': {'$regex': 'End: build Logical Topology'}})
        if topo_end_msg:
            topo_end_log = topo_end_msg.get('msg')
            topo_count = get_log_info(topo_end_log, 5)
            return topo_count
...
def benchmark_data(ben_id, sep_flag, ben_name):       #Benchmark数据统计
    ...
    update_endpoint_time = time_cost(ben_id, 'Begin: update Global Endpoint index.', {'$regex': 'End: update Global Endpoint index.'})
...
if __name__ == '__main__':
    ...
    task_id = "3a0c1d0f-9e62-4d8c-a556-e81331451733"    #task id 自动识别类型
    db_user = "admin"     #mongodb用户名
    db_password = "admin"     #mongodb密码
    db_host = "10.10.7.199"     #mongodb IP
