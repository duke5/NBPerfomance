...
def mpls_cloud_time(act_id):    #根据Benchmark name遍历所有
    mpls_flag = db.BenchmarkSummaryLog.find_one({'dataSourceId': act_id, 'msg': {'$regex': 'End: recalculate Mpls Cloud Route Table.'}})
    if mpls_flag:
        return time_cost(act_id, 'Begin: recalculate Mpls Cloud Route Table.', {'$regex': 'End: recalculate Mpls Cloud Route Table.'})
    else:
        return 'Not Found'
...
def benchmark_data(ben_id, sep_flag, ben_name):       #Benchmark数据统计
    ...
    update_endpoint_time = time_cost(ben_id, 'Begin: update Global Endpoint index.', 'd: update Global Endpoint index. ')
    bgp_mpls_time = time_cost(ben_id, 'Begin: update BGP MPLS cloud CE list.', {'$regex': 'End: update BGP MPLS cloud CE list.'})
    ...
    default_dataview = time_cost(ben_id, 'Begin: build default device data view.', {'$regex': 'End: build default device data view.'})
    pre_qualify_dvt_time = time_cost(ben_id, 'Begin: Pre-qualify data view templates against all devices and logic nodes.',
                                     {'$regex': 'End: Pre-qualify data view templates against all devices and logic nodes.'})
    visual_space_time = time_cost(ben_id, 'Begin: build visual space instance.', {'$regex': 'End: build visual space instance.'})
    ...
    #        live_file.writerow(['Task_type', 'Benchmark'])
            ...
            live_file.writerow([node_count, macdevice_count, intf_count, benchmk_time, benchmk_live, data_folder_size,
                                ...
                                update_endpoint_time, bgp_mpls_time, recal_dyna_devicegp,  recal_site_time, recal_mpls_cloud,
