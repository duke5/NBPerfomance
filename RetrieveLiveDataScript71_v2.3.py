...
def dis_devices(act_id):      #统计当前Discovery device数
    ...
    dis_sum = msg_detail.split('.')[0].split(' ')
    seed_ip = dis_sum[1]
    dis_device = dis_sum[5]
    return str(seed_ip), str(dis_device)
...
def config_coll(act_id, sep_flag):
    ...
    if sep_flag:
        ...
        config_count = coll_sep.find({'dataList.taskId': act_id, 'type': 'config'}).count()
        ...
        config_count = coll.find({'dataList.taskId': act_id, 'type': 'config'}).count()
...
def item_count(act_id, item_name, sep_flag):      #统计不同类型表的count，size
    ...
    if sep_flag:
        coll_sep = sep_db[demonth(act_id)]
        table_count = coll_sep.find({'dataList.taskId': act_id, 'name': item_name}).count()
        size_sum = coll_sep.aggregate([{'$unwind': '$dataList'}, {'$match':
                                                                      {'dataList.taskId': act_id, 'name': item_name}},
                                       {'$group': {'_id': 'null', 'total': {'$sum': '$dataList.len'}}}])
    else:
        table_count = coll.find({'dataList.taskId': act_id, 'name': item_name}).count()
...
def topo_name_specify(act_id, topo_type, less_time):     #拼接可能的topo table name
    if topo_type == "L3_Topo_Type":
        topo_table_source1 = db.BenchmarkSummaryLog.find_one(
            {'dataSourceId': act_id, 'msg': 'Begin: build IPv4 L3 topology.'})
...
def mpls_cloud_time(act_id):    #根据Benchmark name遍历所有
    mpls_flag = db.BenchmarkSummaryLog.find_one({'dataSourceId': act_id, 'msg': 'End: recalculate Mpls Cloud Route Table.'})
    if mpls_flag:
        return time_cost(act_id, 'Begin: recalculate Mpls Cloud Route Table.', 'End: recalculate Mpls Cloud Route Table.')
    else:
        return 'Not Found'
...
def discover_data(dis_id, sep_flag):    #Discovery数据统计
    ...
    node_count = str(db.Device.count())
    device_count = dis_devices(dis_id)
    dis_time = full_time(dis_id)
    dis_live = time_cost(dis_id, 'Begin: discover devices.', 'End: discover devices.')
    data_size = data_size_count(dis_id)[0]
    entries_ct = data_size_count(dis_id)[1]
    config_count = config_coll(dis_id, sep_flag)
    IPv4L3_time = time_cost(dis_id, 'Begin: build IPv4 L3 topology.', 'End: build IPv4 L3 topology.')
    IPv4L3_topo_info = topo_link_count(dis_id, 'L3_Topo_Type', topo_min)
    ...
    sync_site_time = time_cost(dis_id, 'Begin: synchronize with sites.', 'End: synchronize with sites.')
...
def benchmark_data(ben_id, sep_flag, ben_name):       #Benchmark数据统计
    node_count = str(db.Device.count())
    macdevice_count = str(db.MacDevice.count())
    intf_count = str(db.Interface.count())
    benchmk_time = full_time(ben_id)
    benchmk_live = time_cost(ben_id, 'Begin: retrieve devices data.', 'End: retrieve devices data.')
    data_folder_size = data_size_count(ben_id)[0]
    config_count = config_coll(ben_id, sep_flag)
    IPv4Topo_time = time_cost(ben_id, 'Try to build topology IPv4 L3 Topology', {'$regex': 'End: build IPv4 L3 Topology'})
    ...
    ipsec_vpn_topo_time = time_cost(ben_id, 'Try to build topology L3 VPN Tunnel', {'$regex': 'End: build L3 VPN Tunnel'})
    vpn_topo_info = topo_link_count(ben_id, 'VPN_Topo_Type', topo_min)
    topo_logical_time = time_cost(ben_id, 'Try to build topology Logical Topology', {'$regex': 'End: build Logical Topology'})
    topo_logical_info = topo_link_count(ben_id, 'Logical_Topo_Type', topo_min)
    data_view_count = str(db.DefaultDeviceDataView.count())
    devgp_count = str(db.DeviceGroup.count())
    bgp_mpls_time = time_cost(ben_id, 'Begin: update BGP MPLS cloud CE list.', 'End: update BGP MPLS cloud CE list.')
    recal_dyna_devicegp = time_cost(ben_id, 'Begin: recalculate dynamic device group.',
                                    'End: recalculate dynamic device group.')
    recal_site_time = time_cost(ben_id, 'Begin: recalculate site.', 'End: recalculate site.')
    recal_mpls_cloud = mpls_cloud_time(ben_id)
    default_dataview = time_cost(ben_id, 'Begin: build default device data view.', 'End: build default device data view.')
    dv_subtask_time = time_cost(ben_id, {"$regex": "Begin: run data view task with total"},
                                {"$regex": "End: run data view with total"})
    visual_space_time = time_cost(ben_id, 'Begin: build visual space instance.', 'End: build visual space instance.')
    network_tree_time = time_cost(ben_id, 'Begin: build network tree.', 'End: build network tree.')
    ...
    print('---RunDataViewTask subtasks time: ' + dv_subtask_time)
    ...
    title_list = ['Node count', 'MacDevice count', 'Interface count', 'Benchmark time',
                                ...
                                'Device Group count', 'Update BGP MPLS cloud CE list time',
                                ...
                                'Total 24 RunDataViewTask subtasks time', 'Build visual space instance time',
    ...
    #        live_file.writerow(['Task_type', 'Benchmark'])
            ...
            live_file.writerow([node_count, macdevice_count, intf_count, benchmk_time, benchmk_live, data_folder_size,
                                ...
                                bgp_mpls_time, recal_dyna_devicegp,  recal_site_time, recal_mpls_cloud,
                                default_dataview, data_view_count, dv_subtask_time, visual_space_time,
...
def itera_benchmark(benchmk_name, sep_flag):
...
if __name__ == '__main__':
    ...
    domain_name = "5w_dm"   #domain名称
    benchmark_name = ""  #如果配置Benchmark name将直接根据name遍历所有该name的Benchmark，task id配置将无效
    task_id = "f5332ee2-70bf-47e9-be5c-8d02e3e6ce54"    #task id 自动识别类型
    ...
    if benchmark_name:
        ...
        if benchmk_est:
            ...
            itera_benchmark(benchmark_name, is_sep[0])
        else:
